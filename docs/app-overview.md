...
- 出題（`guidedBuildPools`）: 練習モード別にプールを作る。英語は解放済みキーだけで綴れて注目キーを含む
  `EN_WORDS`、日本語は標準ローマ字（`guidedRomajiOf`）が解放済みキーだけで打てる `JP_WORDS`、
  記号は英字部分が解放済みキーだけの `SYM_ITEMS`。不足分は疑似単語・疑似かな・記号テンプレートで補う。
  統計はどの練習モードでも打鍵されたローマ字/英字（a-z）に対して記録される。
